  ...
  return useMemo(() => {
    ...
    droneRoutes.forEach((sortie, sortieIndex) => {
      ...
      droneEvents.push({
        ...
        label: `Sortie ${sortieNum}: Drone Delivery${order?.orderId != null ? ` at Order Point ${order.orderId}` : ''}`,
        orderName: order?.label,
        orderId: order?.orderId,
      ...
      switch (currSig.type) {
        case 'order':
          truckEvents.push({
            ...
            waypointIndex: currSig.index,
            location: currSig.point,
            label: `Truck Delivery${currSig.node?.orderId != null ? ` at Order Point ${currSig.node.orderId}` : ''}`,
            orderName: currSig.node?.label,
            orderId: currSig.node?.orderId,
        ...
        case 'drone_launch': {
          ...
          truckEvents.push({
            ...
            orderId: currSig.orderNode?.orderId,
